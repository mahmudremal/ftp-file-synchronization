        ...
        <div class="card-body">
          <div class="fancy-table table-responsive border rounded">
            <table class="table table-striped mb-0">
              <thead>
                <tr>
                  <th scope="col"><?php esc_html_e( 'Email', 'ftp-file-synchronization' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Profiles', 'ftp-file-synchronization' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Client Status', 'ftp-file-synchronization' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Meeting Time & link', 'ftp-file-synchronization' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Retainer Amount', 'ftp-file-synchronization' ); ?></th>
                  <!-- Contract Options -->
                  <th scope="col"><?php esc_html_e( 'Status', 'ftp-file-synchronization' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Action', 'ftp-file-synchronization' ); ?></th>
                </tr>
              </thead>
              <tbody>
                <?php if( count( $subscribers ) <= 0 ) : ?>
                  <tr>
                    <td colspan="7"><img src="<?php echo esc_url( FTPFILESYNC_BUILD_URI . '/icons/not-found.svg' ); ?>" alt="<?php esc_attr_e( 'Noting found', 'ftp-file-synchronization' ); ?>" style="width: 100%;height: auto;max-height: 400px;"></td>
                  </tr>
                <?php endif; ?>
                <?php foreach( $subscribers as $subscriber ) :
                  ...
                  if( $doc && ! empty( $doc->document_status ) ) {$is_signed = ( $doc->document_status == 'signed' );} else {$is_signed = null;}
                    //   Missing
                  $is_signed = ( $is_signed === null ) ? __( 'Pending', 'ftp-file-synchronization' ) : (
                    ( $is_signed === true ) ? __( 'Signed', 'ftp-file-synchronization' ) : __( 'Pending', 'ftp-file-synchronization' )
                  );
                  $is_active = $activeSubscription ? __( 'Active', 'ftp-file-synchronization' ) : __( 'Paused', 'ftp-file-synchronization' );
                    ...
                    <td>
                      <div class="d-flex align-items-center">
                        <div class="media-support-info">
                          ...
                          <a class="mb-0" href="<?php echo esc_url( $userInfo->meta->meeting_link ); ?>" target="_blank">@<?php esc_html_e( 'Meeting link', 'ftp-file-synchronization' ); ?></a>
                          <?php endif; ?>
                        </div>
                      </div>
                    </td>
