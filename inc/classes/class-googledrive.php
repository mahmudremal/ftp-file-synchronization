...
class GoogleDrive {
	...
	public function developmentMode() {
		...
		try{
			...
			// $drive_file_move = $this->uploadFileWithMeta( FTPFILESYNC_BUILD_PATH . '/img/img-to-contract.jpg' );
	...
	public function submitArchives() {
		...
		if( $result ) {
			...
			wp_send_json_success( [ 'message' => __( 'Archived Successfully.', 'ftp-file-synchronization' ), 'hooks' => [ 'reload-page' ] ], 200 );
		} else {
			wp_send_json_error( __( 'Problem detected while creating archive.', 'ftp-file-synchronization' ), 200 );
		}
	}
	public function deleteArchives() {
		...
		wp_send_json_success( [ 'message' => __( 'Archive removed from server successfully!', 'ftp-file-synchronization' ), 'hooks' => [ 'reload-page' ] ], 200 );
	}
    ...
    if ($http_code == 204) {
			...
			$error_msg = ( $http_code == 404 ) ? __( 'File Not found', 'ftp-file-synchronization' ) : __( 'Failed to delete file from drive.', 'ftp-file-synchronization' );
			throw new \Exception('Error '.$http_code.': ' . $error_msg );
    }
	}
	public function createDirectory( $folderName ) {
		...
		$response = json_decode( $response, true );
		// print_r( $response );
		$error_msg = __( 'Error creating folder:', 'ftp-file-synchronization' ) . ( ! empty( curl_error( $curl ) ) ? ': ' . curl_error( $curl ) : '' );
		curl_close($curl);
		if (curl_errno($curl)) {
